/*
 *  kernel_cfg.c
 *  Wed Oct 28 19:57:48 2015
 *  SG Version 2.00
 *  sg.exe linefollower.oil -os=ECC2 -IC:/cygwin/nxtOSEK/toppers_osek/sg/impl_oil -template=C:/cygwin/nxtOSEK/toppers_osek/sg/lego_nxt.sgt
 */
...
const TaskType task_touch = 1;
const TaskType task_light = 2;

extern void TASKNAME( Task1 )( void );
extern void TASKNAME( task_touch )( void );
extern void TASKNAME( task_light )( void );

static __STK_UNIT _stack_Task1[__TCOUNT_STK_UNIT(1024)];
static __STK_UNIT _stack_task_touch[__TCOUNT_STK_UNIT(512)];
static __STK_UNIT _stack_task_light[__TCOUNT_STK_UNIT(512)];

const Priority tinib_inipri[TNUM_TASK] = { TPRI_MINTASK + 2, TPRI_MINTASK + 3, TPRI_MINTASK + 3, };
const Priority tinib_exepri[TNUM_TASK] = { TPRI_MINTASK + 2, TPRI_MINTASK + 3, TPRI_MINTASK + 3, };
const UINT8 tinib_maxact[TNUM_TASK] = { (1) - 1, (1) - 1, (1) - 1, };
const AppModeType tinib_autoact[TNUM_TASK] = { 0x00000001, 0x00000000, 0x00000000, };
const FP tinib_task[TNUM_TASK] = { TASKNAME( Task1 ), TASKNAME( task_touch ), TASKNAME( task_light ), };
const __STK_UNIT tinib_stk[TNUM_TASK] = { (__STK_UNIT)_stack_Task1, (__STK_UNIT)_stack_task_touch, (__STK_UNIT)_stack_task_light, };
const UINT16 tinib_stksz[TNUM_TASK] = { 1024, 512, 512, };
...
const AlarmType alarm_touch = 0;
const AlarmType alarm_light = 1;

DeclareTask(task_touch);
static void _activate_alarm_alarm_touch( void );
static void _activate_alarm_alarm_touch( void )
{ (void)ActivateTask( task_touch ); }

DeclareTask(task_light);
static void _activate_alarm_alarm_light( void );
static void _activate_alarm_alarm_light( void )
{ (void)ActivateTask( task_light ); }

const CounterType alminib_cntid[TNUM_ALARM] = { 0, 0, };
const FP alminib_cback[TNUM_ALARM] = { _activate_alarm_alarm_touch, _activate_alarm_alarm_light, };
const AppModeType alminib_autosta[TNUM_ALARM] = { 0x00000001, 0x00000001, };
const TickType alminib_almval[TNUM_ALARM] = { 1, 1, };
const TickType alminib_cycle[TNUM_ALARM] = { 250, 10, };
...
const ResourceType lightRes = 0;
const ResourceType runningRes = 1;

const Priority resinib_ceilpri[TNUM_RESOURCE] = { TPRI_MINTASK + 3, TPRI_MINTASK + 3, };
...
const EventMaskType TouchSensorOnEvent = (1UL << 0);
const EventMaskType TouchSensorOffEvent = (1UL << 1);
const EventMaskType ColorSensorEvent = (1UL << 2);
